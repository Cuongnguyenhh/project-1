...
class Product extends CI_Controller
{
    ...
    public function cms_add_product($store_id)
    {
        ...
        if (!empty($check_code) && count($check_code)) {
            ...
            if ($data['prd_code'] == '') {
                $this->db->select_max('prd_code')->like('prd_code', 'PRD');
                $max_product_code = $this->db->get('products')->row();
                $max_code = (int)(str_replace('PRD', '', $max_product_code->prd_code)) + 1;
                if ($max_code < 10)
                    $data['prd_code'] = 'PRD0000' . ($max_code);
                else if ($max_code < 100)
                    $data['prd_code'] = 'PRD000' . ($max_code);
                else if ($max_code < 1000)
                    $data['prd_code'] = 'PRD00' . ($max_code);
                else if ($max_code < 10000)
                    $data['prd_code'] = 'PRD0' . ($max_code);
                else if ($max_code < 100000)
                    $data['prd_code'] = 'PRD' . ($max_code);
            }
